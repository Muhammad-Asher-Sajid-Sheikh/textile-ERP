import {
  createTechPackDraft,
  releaseTechPackVersion,
  createNewVersionFromLocked,
  upsertWeavingSpecification,
  upsertDyeingSpecification,
  addComponentToTechPack,
  removeComponentFromTechPack,
  getTechPackDetails,
  getPendingManagementApprovals
} from '../service/techpack.js';
...
const sendError = (res, error, defaultMessage = 'INTERNAL_SERVER_ERROR') => {
 ...
 */
export const initTechPackDraft = async (req, res) => {
  try {
    ...
    const draft = await techPackService.createTechPackDraft(orderTokenId, {
      designName,
      seasonCode,
      totalTargetQuantity
    });
 ...
 */
export const releaseVersion = async (req, res) => {
  try {
    const { id } = req.params;
    const releasedPack = await techPackService.releaseTechPackVersion(id);
 ...
 */
export const createNewRevision = async (req, res) => {
  try {
    const { id } = req.params;
    const revision = await techPackService.createNewVersionFromLocked(id);
 ...
 */
export const setWeavingSpecs = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const spec = await techPackService.upsertWeavingSpecification(id, weavingData);
 ...
 */
export const setDyeingSpecs = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const spec = await techPackService.upsertDyeingSpecification(id, dyeingData);
    return sendSuccess(res, spec, 200);
  } catch (error) {
    return sendError(res, error);
  }
 ...
 */
export const appendComponent = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const newComponent = await techPackService.addComponentToTechPack(id, componentData);
...
export const deleteComponent = async (req, res) => {
  try {
    ...
    await techPackService.removeComponentFromTechPack(componentId);
 ...
 */
export const fetchTechPackDetails = async (req, res) => {
  try {
    const { identifier } = req.params;
    const details = await techPackService.getTechPackDetails(identifier);
...
export const fetchPendingApprovals = async (req, res) => {
  try {
    const pendingList = await techPackService.getPendingManagementApprovals();
